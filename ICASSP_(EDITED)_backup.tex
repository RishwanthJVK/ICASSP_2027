...
The framework extracts complementary time-domain statistics (mean, variance, standard deviation, RMS, kurtosis, skewness, peak-to-peak amplitude, energy), signal dynamics via Hjorth parameters (Activity, Mobility, Complexity), and complexity descriptors (Shannon, differential, and spectral entropy \cite{zhang2018integration}). Spectral properties were estimated using Welch's PSD \cite{safi2021early} (absolute/relative band power, total power, dominant frequency). Hilbert transform amplitude envelopes \cite{o2017detecting} in $\alpha$ and $\theta$ bands yielded envelope mean, variance, coefficient of variation, and burst count ($\mu+2\sigma$ threshold). Inter-channel synchronization was quantified via pairwise phase-locking value (PLV) and $\alpha$-envelope synchronization:
\begin{equation}
S_i^{\alpha}
=
\frac{1}{C-1}
...
To identify the contribution of individual frequency bands, the feature-
importance values obtained from the Extra Trees analysis were further
aggregated according to frequency band. The usefulness per feature (mean cross-validated importance) associated with frequency band $b$ was calculated as

\begin{equation}
\bar{I}_b
=
\frac{1}{|\mathcal{J}_b|} \sum_{j\in\mathcal{J}_b} I_j,
\end{equation}

where $\mathcal{J}_b$ denotes the set of features associated with frequency
band $b$, $|\mathcal{J}_b|$ is the number of features allocated to that band, and $I_j$ is the cross-validated Gini mean decrease in impurity.
This normalized analysis was used to determine the per-feature information density of
$\delta$, $\theta$, $\alpha$, $\beta$, and $\gamma$ bands for MW--Focus
discrimination.
...
DeepMLP comprised fully connected layers $D\rightarrow1024\rightarrow512\rightarrow256\rightarrow128\rightarrow1$, with batch normalization, GELU
activation\cite{hendrycks2016gaussian},
with dropout rates of $0.50$, $0.40$, $0.30$, and $0.20$, and was trained using weighted binary cross-entropy
with AdamW \cite{loshchilov2017decoupled} optimization. 
...
\begin{figure*}[t]
    ...
    \caption{Mean feature importance across frequency bands for (a) Dataset~1 and
    (b) Dataset~2.}
    \label{fig:band_importance}
\end{figure*}

\textcolor{blue}{Fig.~\ref{fig:band_importance} illustrates the usefulness per feature (mean cross-validated importance, $\bar{I}_b$) across individual frequency bands, providing a normalized comparison that accounts for differing feature allocations per band. In Dataset~1, usefulness per feature is remarkably uniform across all bands: $\gamma$ ($21.64\%$) and $\beta$ ($20.91\%$) exhibited the highest average per-feature utility, closely followed by $\delta$ ($20.64\%$), $\theta$ ($19.36\%$), and $\alpha$ ($17.45\%$). In Dataset~2, $\beta$ features demonstrated the highest usefulness per feature across both raw ($27.78\%$) and artifact-cleaned EEG ($28.11\%$), followed by $\alpha$ ($20.56\%\text{--}21.80\%$) and $\gamma$ ($19.92\%\text{--}20.35\%$), while $\theta$ ($15.91\%\text{--}15.93\%$) and $\delta$ ($14.59\%\text{--}15.06\%$) contributed lower average per-feature importance. These findings demonstrate that when evaluated on a per-feature usefulness basis, high-frequency ($\beta$, $\gamma$) and $\delta$ oscillations exhibit comparable or even superior information density compared to $\alpha$ and $\theta$, confirming that strong discriminative capacity is maintained across both fast and slow oscillatory bands when normalized for feature representation size.}
...
These findings indicate that MW-related EEG differences are not restricted
to static band-power changes, temporal evolution within an epoch provides
additional state-discriminative information.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table}[h]
\centering
\caption{Ablation analysis using SVM on raw EEG}
...
This study presented an integrated multi-domain EEG framework characterizing mind wandering across two datasets differing in montage density and task paradigm. By evaluating per-feature usefulness, we demonstrated that discriminative information is distributed across complementary representations, with Hjorth parameters and high-frequency ($\beta$, $\gamma$) oscillations exhibiting high information density despite compact dimensionalities. Systematic ablation confirmed that the most informative feature family and frequency band varied across paradigms, while integrated multi-domain representations enabled linear models to match deep architectures at up to 88.06\% accuracy. These findings confirm that MW-related neural signatures are shaped by paradigm-dependent cognitive processes and underscore the necessity of multi-dimensional characterization over single-domain approaches.
